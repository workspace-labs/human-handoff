...
- **DO NOT ACCEPT** — only when the supplied evidence itself establishes a blocking defect, a failed requirement or an unmet acceptance condition. Then stop.
...
DO NOT ACCEPT always ends with exactly this line, even when the handoff already carries a corrective prompt: `Your call. If you agree with the rejection, say so and I will write the corrective prompt.`

## Human gate

The human decides. Never present a recommendation as the human's decision, and never write it as "accepted" or "rejected".

After DO NOT ACCEPT, write no corrective prompt until the human explicitly agrees with the rejection, for example "yes, reject it" or "write the fix prompt". Silence, a question or a new topic is not agreement. If the handoff already carries a corrective prompt or repair brief, do not repeat it, offer it, or call it ready or approved before then. If the human decides otherwise, that decision stands.

Once the human agrees, exactly one corrective output follows. If the handoff comes from a skill with its own corrective format, such as independent-review-handoff's CLAUDE CORRECTIVE HANDOFF, use that format and write no prompt of your own: reply with the handoff's own version unchanged if it has one, write it by that skill's rules with every detail they require if that skill is available here, or else tell the human to ask the agent that wrote the handoff for it. Otherwise, reply with one concise corrective prompt they can copy, in this shape:
...
Do not send it, run it or act on it. Write it once; write another version only when the human asks.
